...
np.save("arxiv_dataset/SBERTEmbeddings_final.npy", sbert_embeddings)
print(f"Saved SBERTEmbeddings_final.npy with shape {sbert_embeddings.shape}")

# ================== for test set
df = pd.read_csv("finetune_BERT/test_content.csv")
texts = df["content"].tolist()
n = len(texts)
print(f"Loaded {n} documents")
...
sbert_embeddings = sbert_embeddings.astype("float32")
print(sbert_embeddings.shape)
np.save("arxiv_dataset/SBERTEmbeddings_test_final.npy", sbert_embeddings)
print(f"Saved SBERTEmbeddings_test_final.npy with shape {sbert_embeddings.shape}")
